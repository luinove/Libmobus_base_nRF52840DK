        ...
        if (length_to_read == 0) {
            ...
            case _STEP_FUNCTION:
                ...
                length_to_read = compute_meta_length_after_function(
                    msg[ctx->backend->header_length], msg_type); // return default 1 
                if (length_to_read != 0) {
                    step = _STEP_META;
                    break;
...
static int check_confirmation(modbus_t *ctx, uint8_t *req, uint8_t *rsp, int rsp_length)
{
    ...
    rsp_length_computed = compute_response_length_from_request(ctx, req);// return 7
    ...
    if ((rsp_length == rsp_length_computed ||
        ...
        case MODBUS_FC_READ_INPUT_REGISTERS:
            ...
            req_nb_value = (req[offset + 3] << 8) + req[offset + 4];//1
            rsp_nb_value = (rsp[offset + 1] / 2); //1 
            break;
        case MODBUS_FC_WRITE_MULTIPLE_COILS:
        case MODBUS_FC_WRITE_MULTIPLE_REGISTERS:
...
static int read_registers(modbus_t *ctx, int function, int addr, int nb, uint16_t *dest)
{
    ...
    rc = send_msg(ctx, req, req_length); // in this step rc = 8 the number of send characters
    if (rc > 0) {
        unsigned int offset;
        int i;

        rc = _modbus_receive_msg(ctx, rsp, MSG_CONFIRMATION); // in this step we should have the right number of received characters
        if (rc == -1)
            return -1;

        rc = check_confirmation(ctx, req, rsp, rc);// in this step we should have the right number of received characters
        if (rc == -1)
            return -1;
